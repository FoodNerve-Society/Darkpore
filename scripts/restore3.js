  ...
  try {
    ...
    if (mapData.sections) {
      for (const section of mapData.sections) {
        if (section.map && section.map.sourcesContent) {
          for (let i = 0; i < section.map.sourcesContent.length; i++) {
            const content = section.map.sourcesContent[i];
            if (content && content.includes('SPIKY_TITLE_TEMPLATES')) {
              console.log(`Found original in ${mapPath} section at index ${i}`);
