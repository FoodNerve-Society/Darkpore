...
import { Box, Typography, Card, CardContent, Button, Chip, Divider, IconButton, Avatar, TextField } from '@mui/material';
...
export default function UpdatesPage() {
  ...
  return (
    <Box sx={{ maxWidth: '800px', mx: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {['All', 'Following', 'Trade', 'Learn', 'Meet'].map(filter => (
            <Chip 
              ...
              sx={{ 
                ...
                bgcolor: activeFilter === filter ? 'primary.main' : 'rgba(0,0,0,0.05)',
                color: activeFilter === filter ? 'white' : 'text.primary',
                '&:hover': { bgcolor: activeFilter === filter ? 'primary.dark' : 'rgba(0,0,0,0.1)' }
              }} 
            />
          ))}
        </Box>
      ...
      {/* Main Feed Area */}
      {!hasUpdates ? (
        <Card component={motion.div} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} sx={{ borderRadius: 4, border: '1px dashed rgba(0,0,0,0.15)', boxShadow: 'none', bgcolor: 'transparent' }}>
          <CardContent sx={{ textAlign: 'center', py: 8 }}>
            <RssFeedIcon sx={{ fontSize: 60, color: 'rgba(0,0,0,0.1)', mb: 2 }} />
            ...
            <Divider sx={{ mb: 4 }}><Chip label="CURATE YOUR FEED" size="small" /></Divider>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, justifyContent: 'center', maxWidth: 600, mx: 'auto' }}>
              {availableNiches.map(niche => (
                <Chip
                  ...
                  icon={selectedNiches.includes(niche) ? <ControlPointIcon sx={{ transform: 'rotate(45deg)' }} /> : <ControlPointIcon />}
                  sx={{
                    px: 1, py: 2.5, borderRadius: 2,
                    fontWeight: 700,
                    border: '1px solid',
                    borderColor: selectedNiches.includes(niche) ? 'primary.main' : 'rgba(0,0,0,0.1)',
                    bgcolor: selectedNiches.includes(niche) ? 'primary.50' : 'background.paper',
                    color: selectedNiches.includes(niche) ? 'primary.main' : 'text.secondary',
            ...
            <Box sx={{ mt: 5 }}>
              <Button 
                ...
                sx={{ borderRadius: 8, px: 6, py: 1.5, fontWeight: 800 }}
              >
