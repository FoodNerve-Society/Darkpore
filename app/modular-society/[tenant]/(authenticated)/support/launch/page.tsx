...
export default function LaunchCampaignPage() {
  ...
  const handleNext = () => {
    if (activeStep === STEPS.length - 1) {
      setSubmitted(true);
      setTimeout(() => router.push("/society/support"), 2000);
    } else {
  ...
  if (submitted) {
    return (
      <Box sx={{ p: 4, textAlign: "center", mt: 8 }}>
        <Paper sx={{ ...glassCard, p: 5, maxWidth: 500, mx: "auto" }}>
          <RocketLaunchIcon sx={{ fontSize: 64, color: GREEN, mb: 2 }} />
          <Typography variant="h5" sx={{ fontWeight: 900, mb: 2 }}>
            Campaign Launched!
          </Typography>
          <Typography color="text.secondary">
            Redirecting you to the support hub...
          </Typography>
        </Paper>
      </Box>
    );
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 640, mx: "auto" }}>
      <Button startIcon={<ArrowBackIcon />} onClick={() => router.back()} sx={{ mb: 2, color: "text.secondary", fontWeight: 700 }}>
        Back
      </Button>
