...
const CATEGORY_OPTIONS: {
  ...
  },
  {
    ...
    label: "Flash Sale",
    emoji: "⚡",
    description: "Sell perishable or surplus goods quickly before they expire",
  },
  {
    value: "group-buy",
    label: "Group-Buy (Ajo)",
    emoji: "🤝",
    description: "Pool resources with others to share costs on big purchases",
  },
  {
    value: "swap",
    label: "Swap",
    emoji: "♻️",
    description: "Trade goods directly without cash — barter economy style",
  },
  {
    value: "need",
    label: "Need",
...
export default function CreateListingPage() {
  ...
  // ── Rank 2 Gate — Beautiful Upgrade Prompt ────────────
      ...
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 600, mx: "auto" }}>
        ...
        >
          ...
          <Box sx={{ display: "flex", gap: 1.5, justifyContent: "center", alignItems: "center", mb: 4, flexWrap: "wrap" }}>
            <Chip
              icon={<ShieldIcon sx={{ fontSize: "15px !important", color: `${RANK_COLORS[gate.currentRank as RankLevel]} !important` }} />}
              label={`Current: ${RANK_NAMES[gate.currentRank as RankLevel]}`}
              sx={{ fontWeight: 700, fontSize: "0.82rem", height: 36, bgcolor: alpha(RANK_COLORS[gate.currentRank as RankLevel], 0.15), color: RANK_COLORS[gate.currentRank as RankLevel], border: `1px solid ${alpha(RANK_COLORS[gate.currentRank as RankLevel], 0.3)}` }}
            />
            <Typography variant="h6" sx={{ color: "text.disabled", fontSize: "1.2rem" }}>→</Typography>
            <Chip
              icon={<StarIcon sx={{ fontSize: "15px !important", color: `${RANK_COLORS[gate.requiredRank as RankLevel]} !important` }} />}
              label={`Needed: ${RANK_NAMES[gate.requiredRank as RankLevel]}`}
              sx={{ fontWeight: 700, fontSize: "0.82rem", height: 36, bgcolor: alpha(RANK_COLORS[gate.requiredRank as RankLevel], 0.15), color: RANK_COLORS[gate.requiredRank as RankLevel], border: `1px solid ${alpha(RANK_COLORS[gate.requiredRank as RankLevel], 0.3)}` }}
            />
          </Box>
          ...
          <Button
            variant="contained"
            fullWidth
            onClick={() => router.push(gate.upgradeRoute || "/profile/setup")}
            sx={{ background: `linear-gradient(135deg, ${EMERALD} 0%, ${EMERALD_DARK} 100%)`, color: "#ffffff", fontWeight: 800, fontSize: "1rem", py: 1.8, borderRadius: "16px", textTransform: "none", boxShadow: `0 6px 24px ${alpha(EMERALD, 0.35)}`, "&:hover": { background: `linear-gradient(135deg, ${EMERALD_DARK} 0%, #047857 100%)` } }}
          >
            Upgrade Now
          </Button>
        </Paper>
    ...
    return (
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 500, mx: "auto", textAlign: "center", mt: { xs: 4, md: 8 } }}>
        <Paper elevation={0} sx={{ ...glassCard, p: { xs: 4, md: 5 } }}>
          ...
          <Typography variant="h5" sx={{ fontWeight: 900, mb: 1, fontSize: "1.4rem", color: "#000" }}>
            Listing Published! 🎉
          </Typography>
          <Typography variant="body1" sx={{ color: "text.secondary", mb: 2, lineHeight: 1.6 }}>
            Your listing is now live on the marketplace. Redirecting you back...
          </Typography>
        </Paper>
      </Box>
    );
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 640, mx: "auto", color: "#000" }}>
      <Button startIcon={<ArrowBackIcon />} onClick={() => router.back()} sx={{ mb: 2, color: "text.secondary", fontWeight: 700, textTransform: "none", borderRadius: "12px", "&:hover": { bgcolor: alpha(EMERALD, 0.08) } }}>
        Back
      </Button>
      ...
      <Typography variant="body2" sx={{ color: "text.secondary", mb: 3 }}>
        List something on the marketplace — it takes less than 2 minutes.
      </Typography>
      ...
      <Paper elevation={0} sx={{ ...glassCard, p: { xs: 3, md: 4 } }}>
        {activeStep === 0 && (
          <Box>
            ...
            <TextField fullWidth label="Listing Title" value={title} onChange={(e) => setTitle(e.target.value)} sx={{ mb: 3 }} />
          </Box>
        )}
        {activeStep === 1 && (
          <Box>
