...
export function generateStaticParams() {
  const slugs: { challenge: string }[] = [];
  Object.values(TENANTS).forEach((tenant) => {
    tenant.com.homepage.challenges.forEach((w) => slugs.push({ challenge: w.id }));
  });
  return slugs;
}

export default async function ChallengeLearnHub({ params }: { params: Promise<{ challenge: string }> }) {
  ...
  if (!challengeData) {
    return <div style={{ padding: '4rem', textAlign: 'center' }}>Challenge not found.</div>;
  }
  ...
  return (
    <Box sx={{ minHeight: '100vh', pb: 15, bgcolor: 'background.default' }}>
      ...
      <Box sx={{ bgcolor: 'secondary.main', color: 'secondary.contrastText', pt: 15, pb: 10, px: 2 }}>
        <Container maxWidth="lg">
          ...
          <Typography variant="h6" sx={{ opacity: 0.9, maxWidth: '800px' }}>
            Explore our curated intelligence on {challengeData.desc.toLowerCase()}
          </Typography>
        </Container>
      </Box>
      ...
      <Container maxWidth="lg" sx={{ mt: -6, position: 'relative', zIndex: 10 }}>
        ...
        {/* Featured Content */}
        {featured && (
          <Link href={`/${challenge}/learn/${featured.slug}`} passHref style={{ textDecoration: 'none' }}>
            <Card sx={{ 
              display: 'flex', 
        ...
        <Typography variant="h5" sx={{ fontWeight: 800, mb: 4 }}>More {challengeData.title} Intelligence</Typography>
        <Grid container spacing={4}>
          {others.map((material, idx) => (
            <Grid size={{ xs: 12, md: 4 }} key={idx} sx={{ p: 2 }}>
              <Link href={`/${challenge}/learn/${material.slug}`} passHref style={{ textDecoration: 'none' }}>
                <Card sx={{ 
                  height: '100%', 
                  borderRadius: 4, 
                  display: 'flex', 
