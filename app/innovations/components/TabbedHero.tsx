...
function GlobalAlertBanner({ alerts, activeCategoryId }: { alerts: any[], activeCategoryId?: string }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Wrap at length + 1 so the last card has time to enter the "Past" state
      setCurrentIndex(prev => (prev + 1) % (filteredAlerts.length + 1));
    }, 6000);
        ...
        <Box sx={{ maxWidth: { md: '850px' }, mx: { md: 'auto' } }}>
          ...
          animate={{
            height: filteredAlerts.length > 0
              ? 52 + (Math.min(currentIndex, filteredAlerts.length - 1) * 64) + (Math.max(0, filteredAlerts.length - 1 - currentIndex) * 10)
              : 52
          }}
          ...
          <AnimatePresence>
            {filteredAlerts.map((currentAlert, i) => {
              ...
              // -------------------------------------------------------------
              // CRITICAL ANIMATION FIX: "The Deal"
              // Older cards (lower i) must ALWAYS have a higher z-index. 
              // This ensures that when a new card slides down to become active, 
              // it slides OUT FROM UNDERNEATH the previous card, rather than 
              // instantly popping to the front and ruining the illusion.
              // -------------------------------------------------------------
              zIndex = filteredAlerts.length - i; 
              ...
              const hasImage = !!currentAlert.imageUrl;
              ...
              if (isActive) {
                ...
                cardBlur = 'none';
              } else if (isPast && hasImage) {
                // Past cards with images become cinematic full-image backgrounds
                // We use a dark gradient on the left to ensure the status badge and text remain perfectly readable
                cardBg = `linear-gradient(90deg, rgba(15,23,35,0.95) 0%, rgba(15,23,35,0.6) 40%, rgba(15,23,35,0.8) 100%), url(${currentAlert.imageUrl}) center/cover no-repeat`;
                cardBlur = 'none';
              }
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', pt: 2.5 }}>
            <Box sx={{
              ...
              <Typography sx={{
                ...
                {String(Math.min(currentIndex + 1, filteredAlerts.length)).padStart(2, '0')}
              </Typography>
              <Typography sx={{
                fontSize: '0.65rem',
                fontWeight: 400,
