...
import type { ChallengeUpdate, LearningMaterial } from '@/lib/cms/types';

interface Props {
  challengeId: string;
  feedUpdates: ChallengeUpdate[];
...
function UpdateCard({ update, challengeId }: { update: ChallengeUpdate; challengeId: string }) {
  return (
    <Card sx={{ bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 4, overflow: 'hidden', transition: 'all 0.35s', '&:hover': { transform: 'translateX(6px)', borderColor: 'rgba(255,255,255,0.18)', bgcolor: 'rgba(255,255,255,0.055)' } }}>
      <Link href={`/${challengeId}/${update.section}`} passHref style={{ textDecoration: 'none', color: 'inherit' }}>
        <Box sx={{ p: { xs: 3, md: 4 } }}>
...
function MaterialCard({ material, challengeId }: { material: LearningMaterial; challengeId: string }) {
  return (
    <Link href={`/${challengeId}/learn/${material.slug}`} passHref style={{ textDecoration: 'none', color: 'inherit' }}>
...
export default function ChallengeDashboardTabs({ challengeId, feedUpdates, learningMaterials }: Props) {
        ...
        {activeTab === 'intelligence' ? (
          <Box sx={{ position: 'relative' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              {learningMaterials.slice(0, VISIBLE_COUNT).map((m) => <MaterialCard key={m.slug} material={m} challengeId={challengeId} />)}
            ...
            <ContinueButton href={`/${challengeId}/learn`} label="View Knowledge Center →" />
          </Box>
        ) : (
          <Box sx={{ position: 'relative' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              {currentItems.slice(0, VISIBLE_COUNT).map((u) => <UpdateCard key={u.id} update={u} challengeId={challengeId} />)}
            ...
            {activeTabObj?.section && <ContinueButton href={`/${challengeId}/${activeTabObj.section}`} label={`Continue in ${activeTabObj.label} →`} />}
          </Box>
        )}
      </Box>
    </Box>
