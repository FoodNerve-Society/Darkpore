...
export default async function ChallengesManifestoPage() {
    ...
    <Box sx={{ bgcolor: '#000', minHeight: '100vh' }}>
      ...
      >
        ...
        <Container maxWidth="md" sx={{ position: 'relative', zIndex: 2 }}>
          ...
          >
            THE {homepageConfig.challenges.length} CHALLENGES
          </Typography>
          <Typography
            variant="h2"
            sx={{
      ...
      ═══════════════════════════════════════════════════════════════ */}
      {homepageConfig.challenges.map((challenge, idx) => {
        ...
        return (
          <React.Fragment key={challenge.id}>
            ...
            <Link href={`/${challenge.id}`} passHref style={{ textDecoration: 'none', color: 'inherit' }}>
              <Box
                sx={{
                  position: 'relative',
                  ...
                  cursor: 'pointer',
                  transition: 'all 0.4s',
                  '&:hover .challenge-img': { transform: 'scale(1.03)', filter: 'brightness(0.35) saturate(1.3)' },
                }}
              >
                {/* Background image */}
                <Box
                  ...
                  <Grid container spacing={6} sx={{ alignItems: 'center' }}>
                    ...
                    >
                      <Grid container spacing={2}>
                        {[
                          { label: 'Active Solutions', value: String(challenge.stats.activeSolutions), icon: <RocketLaunchIcon sx={{ fontSize: 20, color: 'primary.light' }} /> },
                          { label: 'Capital Deployed', value: challenge.stats.capitalDeployed, icon: <TrendingUpIcon sx={{ fontSize: 20, color: '#f59e0b' }} /> },
                          { label: 'Community', value: challenge.stats.communitySize, icon: <GroupsIcon sx={{ fontSize: 20, color: '#6366f1' }} /> },
                        ].map((stat, sidx) => (
                          <Grid size={{ xs: 12, sm: 4, md: 12 }} key={sidx}>
                            <Card
                              sx={{
      ...
      >
        <Container maxWidth="sm" sx={{ position: 'relative', zIndex: 2 }}>
          ...
          <Typography variant="h3" sx={{ fontWeight: 900, color: '#fff', mb: 3, fontSize: { xs: '1.8rem', md: '2.5rem' }, lineHeight: 1.2 }}>
            See a challenge we missed?
          </Typography>
          <Typography sx={{ color: 'rgba(255,255,255,0.45)', mb: 5, fontSize: '1.1rem', lineHeight: 1.7 }}>
            The challenge map is a living document built by the Society. If you've identified a systemic failure we haven't cataloged, submit it.
          </Typography>
          <Button
            variant="outlined"
            size="large"
